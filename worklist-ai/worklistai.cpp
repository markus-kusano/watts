...
struct WorklistAI : public ModulePass {
  ...
  void merge_worklist(
    ...
    for (auto p = ps.begin(), pe = ps.end(); p != pe; ++p) {
      // Check if any of the basicblock are already in worklist. Merge if they
      // are.
  ...
  bool stableStores(
    ...
    // Since the store instructions are unique (not shared across threads) we
    // can flatten the results and just work on the combined inner maps
  ...
  std::map<BasicBlock *, LatticeFact> analyzeFuncNoInterf(ap_manager_t *man
    ...
    return analyzeFunc(man, env, empty, rErrors, reachableStores
        , reachThreadCreate, assertSlice, impacted, mayImpact, valToName, entryFact, f, false);
  }
  ...
  // environment should atleast include every variable in the passed map from
  ...
  std::map<BasicBlock *, LatticeFact> analyzeFunc(ap_manager_t *man
    ...
    // The current state of the analysis. This is the facts at the begining of
    // each basicblock. If a key is not in the map it is assumed to be an
    // error.
    ...
    while (worklist.size()) {
      try {
        DEBUG_MSG("New worklist iteration, size: " << worklist.size() << '\n');
        ...
        // If the new fact is the same size or smaller than the old one then we
        // can stop. It should never really be smaller (assuming the transfer
        // functions are monotonicly increasing).
        if (newFact.leq(oldFact)) {
          continue;
        }
        // If the new fact is bigger, save it in the analysis state Note: the
        // end fact of each block is not saved (but, it can be recovered by
        // just running the transfer function over the input fact)
        ...
        std::vector<std::pair<BasicBlock*, LatticeFact> > outFacts
          = BBOps::run_transfer_funcs(curBb, newFact, interf, rErrors
              , reachableStores, reachThreadCreate, assertSlice, impacted, mayImpact, valToName
							, noCombinsG);
  ...
  void addDomLoadStoreFacts(std::set<Function*> threadFuncs
    ...
    for (Function *f : threadFuncs) {
      ...
      for (BasicBlock &b : *f) {
        addInternalDomFacts(&b, ctx, zfp);
        addLoadStoreFacts(&b, ctx, zfp);
        DomTreeNodeBase<BasicBlock> *bbNode = dt.getNode(&b);
        if (bbNode == NULL) {
          continue;
        }
        // For the dom children of `b`, connect the terminator of `b` to the
        // first relevant (load, store, terminator) instruction in the child
  ...
  bool runOnModule(Module &M) {
    ...
    do {
      ...
      // The reachable stores for this round of the analysis. We save this in a
      // new map so we can compare it with the results of the previous analysis
      // (allReachStores). If the facts stabalize, then we can stop.
      std::map<Function *, std::map<Instruction *, LatticeFact>> newReachStores 
        = allReachStores;
      ...
      // This means that each iteration starts analyzing main and then analyzes
      // the children of main recursively.
      ...
      while (threadsToProcess.size()) {
        ...
        if (dynInitG) {
          // If we are dynamically calculating the entry state of each thread,
          // then we retrieve the entry fact here. Otherwise, the entry fact is
          // simply initialized to be the initial state of the program
          if (ci != NULL) {
            entryFact = Utils::mapAtUnsafe(threadEntryFacts, ci);
          }
        }
        // Subset of all interferences. Only those which could affect this
        // thread.
        std::map<Instruction *, std::vector<Instruction *>> curFInterf = interfs[f];
        if (filterMHB) {
          curFInterf = filterMHBInterfs(ctx, zfp, curFInterf);
        }
        // A map of reachable thread creations in `f`. This is used to
        // determine other threads which should be analyzed
        std::map<CallInst *, LatticeFact> reachThreadCreate;
      ...
      // store instructions
      if (!stable) {
        // Calculate the interferences to be used for each thread to be used in
        // the next round of the analysis. This uses the reachable stores
        // caluclated for each thread in the previous for-loop.
        for (Function *f : threadFuncs) {
          ...
          interfStores = getInterferingStores(f, newReachStores, valToName);
					...
					if (assertSliceG || impactG) {
						for (auto it = interfStores.begin(), ie = interfStores.end()
								; it != ie; ++it) {
              // assertSlice contains those statements *not* on the slice.
              // So, if it is found in the assertSlice structure then the
              // statement must not be on the slice. So, its interference can
              // be removed.
              // TODO: for some reason performing this removal causes some
              // false alarms??
              //
              // These should be mutually exclusive so as not to double erase
  ...
  z3::expr getValueBVID(z3::context &ctx, Value *v) const {
    ...
    //static_assert(sizeof(unsigned) * 8 >= Z3_BV_SIZE
    //    , "Bitvector size cannot fit in unsigned");
    //static std::map<Value *, unsigned> cache;
    //auto hit = cache.find(v);
    //unsigned id;
    //if (hit != cache.end()) {
    //  id = hit->second;
    //}
    //else {
    //  id = cache.size();
    //  if (std::ceil((std::log(id) / std::log(2))) > Z3_BV_SIZE) {
    //    errs() << "[ERROR] Overflowing Bitvector size, increase it: "
    //           << "current size: " << Z3_BV_SIZE << '\n';
    //    DEBUG_MSG("ID: " << id << '\n');
    //    DEBUG_MSG("required bits: " << std::ceil((std::log(id) / std::log(2))));
    //    exit(EXIT_FAILURE);
    //  }
    //  cache[v] = id;
    //}
    //z3::expr ret = ctx.bv_val(id, Z3_BV_SIZE);
    //return ret;
  }
  ...
  // Return the dominance facts within the basicblock (i.e., the order of
  ...
  void addInternalDomFacts(BasicBlock *b, z3::context &ctx, Z3_fixedpoint &zfp) {
    ...
    if (loadsNStores.size() == 1) {
      // With only one instruction (the terminator) there are no internal
      // dominance facts
      return;
    }
