...
get_num_errs() {
  if [ -z "${1:-}" ]
  then
    die "ERROR: get_num_errs(): $1 is empty"
  fi

  if [ ! -f "${1:-}" ]
  then
    die "ERROR: get_num_errs(): ${1:-} is not a file"
  fi

  errString=$(grep 'Errors found: ' $1)
...
run_box_imp_tso() {
  date
  echo "$TIMEOUT $TIME -o box_imp_tso.time $OPT -load $WORKLIST_SO -worklist-ai $AI_ARGS -tso -impact -aslice -constraints -z3 $Z3_BIN -box main_impact.ll >main_impact_out.bc 2>box_imp_tso.out"
  $TIMEOUT $TIME -o box_imp_tso.time $OPT -load $WORKLIST_SO -worklist-ai $AI_ARGS -tso -impact -aslice -constraints -z3 $Z3_BIN -box main_impact.ll >main_impact_out.bc 2>box_imp_tso.out 
  ret=$?
  check_timeout $ret box_imp_tso.out "[ERROR]: opt (box imp tso)" 
  check_results box_imp_tso
}

run_box_imp_pso() {
  date
  echo "$TIMEOUT $TIME -o box_imp_pso.time $OPT -load $WORKLIST_SO -worklist-ai $AI_ARGS -pso -impact -aslice -constraints -z3 $Z3_BIN -box main_impact.ll >main_impact_out.bc 2>box_imp_pso.out"
  $TIMEOUT $TIME -o box_imp_pso.time $OPT -load $WORKLIST_SO -worklist-ai $AI_ARGS -pso -impact -aslice -constraints -z3 $Z3_BIN -box main_impact.ll >main_impact_out.bc 2>box_imp_pso.out 
  ret=$?
  check_timeout $ret box_imp_pso.out "[ERROR]: opt (box imp pso)" 
  check_results box_imp_pso
}

run_box_imp_rmo() {
  date
  echo "$TIMEOUT $TIME -o box_imp_rmo.time $OPT -load $WORKLIST_SO -worklist-ai $AI_ARGS -rmo -impact -aslice -constraints -z3 $Z3_BIN -box main_impact.ll >main_impact_out.bc 2>box_imp_rmo.out"
  $TIMEOUT $TIME -o box_imp_rmo.time $OPT -load $WORKLIST_SO -worklist-ai $AI_ARGS -rmo -impact -aslice -constraints -z3 $Z3_BIN -box main_impact.ll >main_impact_out.bc 2>box_imp_rmo.out 
  ret=$?
  check_timeout $ret box_imp_rmo.out "[ERROR]: opt (box imp rmo)" 
  check_results box_imp_rmo
}
